...
model_dir = './result/model-morp-sum-seq-norestrict'
...
def eval_dev(my_model):
    ...
    with torch.no_grad():
        ...
        for s_idx in range(len(dev)):
            ...
            tag_scores = my_model(input_sent, dp_in)            
...
class LSTMTagger(nn.Module):
    ...
    def forward(self, input_sent, dp_in):
...
#         LSTM layer
        if DPINFO:
            input_embs = torch.cat( (tok_vec, dp_embs), 1)
        else:
            input_embs = tok_vec  
...
for epoch in range(NUM_EPOCHS):
    ...
    for s_idx in range(len(trn)):
        ...
        tag_scores = srl_model(input_sent, dp_in)
